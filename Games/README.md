...
UPDATE:
- Added a more intelligent guessing algorithm. (Medium/Hard difficulty. Will not attack both two ships at once if they are next to each other. To clarify, if two ships are right next to each other and the computer manages to discover both of them in a short time frame, it will only attempt to fully sink one of them before returning to random guessing)
