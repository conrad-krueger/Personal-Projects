...
limit = len(word_list)
while True:
  pick = randint(0, limit - 1)
  if (len(word_list[pick]) > 5):
    qual = len(word_list[pick])
    for x in word_list[pick]:
      if (x == "-" or  x == " "):
        qual -= 1
    if (qual == len(word_list[pick])):
      break
...
trigger = True #prevents redrawing
...
#turtle for depicting number of lives
turTries = turtle.Turtle()
turTries.hideturtle()
turTries.speed('fastest')
turTries.penup()
turTries.goto(175,255)
turTries.pendown()

#turtle for depicting word guesses
turWord = turtle.Turtle()
turWord.hideturtle()
turWord.speed('fastest')
turWord.penup()
turWord.goto(-400,-125)
turWord.pendown()

#turtle for depicting letters already tried
turGuess = turtle.Turtle()
turGuess.hideturtle()
turGuess.speed('fastest')
turGuess.penup()
turGuess.goto(-400,-175)
turGuess.pendown()

#Depicts player guesses, how much of the word has been solved, and how many lives left
def preguess(tries, depict, guessed_letters):
  turTries.clear()
  turTries.write(f"You have {tries} lives",font=("Courier", 20, "bold"))
  hanging(tries)
  solved = ""
  for q in depict:
    solved+= (q + " ") 
  turWord.clear()
  turWord.write(solved,font=("Courier", 30, "bold"))
  
  
  guess = "Guessed letters/words: "
  
  for e in guessed_letters:
    guess += (e + " ")
  turGuess.clear()
  turGuess.write(guess,font=("Courier", 15, "normal"))



#turtle for depicting if current guess is correct
turAlpha = turtle.Turtle()
turAlpha.hideturtle()
turAlpha.speed('fastest')
turAlpha.penup()
turAlpha.goto(-375,125)
turAlpha.pendown()

#compares letter or word guess to actual word
def guess(attempt, secret):
  ...
  if (correct == 0):
    turAlpha.color("red")
    turAlpha.write("X", font=("Courier",80,"bold"))
    time.sleep(1.59)
    turAlpha.clear()
    return 0
...
def main():
    ...
    while True:
      global depict
      global winner
      preguess(lives, depict, guessed_letters)

      #trigger winning screen
      if (winner):
          time.sleep(2)
          turAlpha.color("green")
          turAlpha.write("YOU WIN!", font=("Courier",80,"bold"))
          time.sleep(2.25)
          turAlpha.clear()
          break
      
      #trigger losing message
      if (lives == 0):
          turAlpha.color("red")
          turAlpha.write(f"You died... \n The word was: {word}", font=("Courier",30,"bold"))
          time.sleep(2.25)
          turAlpha.clear()
          break
      
      #prompt user for guess in terminal
      while True:
          tracker = 0
          attempt = input("Choose a letter or guess the word: ")
          if (attempt == "?"):
            x = input("Does the black moon howl? ")
            if (x == "Yes, but only in winter."):
                print(word)
                continue
          for a in guessed_letters:
            if (attempt == a):
              tracker += 1
          if (tracker == 0):
            break
            ...
            time.sleep(1.75)
            turAlpha.color("black")
            turAlpha.write("Oh. You seem to have choosen \n a letter or word that you \n have already tried. Try Again", font=("Courier",20,"bold"))
            time.sleep(2.1)
            turAlpha.clear()
      #update fill in the blank screen based on guesses
      calc = guess(attempt, secret)
      if (calc != 0):
          for c in calc:
            depict[c] = attempt
      if (calc == []):
