 ...
 */
DenseNeuralNetwork::DenseNeuralNetwork(string loss, vector<size_t> numOfNodes, vector<string> activationPerLayer): delta(), loss(loss), weights(), nodes(), biases(),layerTotal(numOfNodes.size()), nodeTotal(), weightTotal(), nodePerLayer(numOfNodes), activationPerLayer(activationPerLayer) {
    ...
    if (loss != "mean squared error" && loss != "cross entropy")
        throw "ERROR: Dense Neural Network Class version 2.0 only accepts \"mean squared error\" or \"cross entropy\" as loss functions";
...
void DenseNeuralNetwork::update(double learning_rate, vector<double> expected){
    ...
    for (size_t j = 0; j < nodePerLayer.at(i); j++){ //compare each of the output nodes to their expected values * derivative of activation function
        ...
        if (loss == "cross entropy")
            delta[i-1][j] = -1*(expected.at(j)/nodes[i][j]) + (1-expected.at(j))/(1-nodes[i][j]); //part deriv of cross entropy 
        
        if (loss == "mean squared error")
            delta[i-1][j] = (nodes[i][j] - expected.at(j));

        //Derivative of Activation Function

        if(activationPerLayer.at(i-1) == "logistic" || activationPerLayer.at(i-1) == "softmax"){ //questioning this
            delta[i-1][j] *= (nodes[i][j] * (1-nodes[i][j]));

        }else if(activationPerLayer.at(i-1) == "relu"){
            if (nodes[i][j] < 0) delta[i-1][j] = 0;
        
        }else if(activationPerLayer.at(i-1) == "tanh"){
            delta[i-1][j] *= (1 - nodes[i][j]*nodes[i][j]);
            
        }else if(activationPerLayer.at(i-1) != "linear"){
 ...
 * @return None
 */
void DenseNeuralNetwork::computeLayer(size_t layer, string activation){ //sofar good
    if (layer >= layerTotal)
    ...
    if (activation == "relu"){
        ...
        activate = tanh;
    }else if(activation != "linear" && activation != "softmax"){
        throw "ERROR: Must use on of the following Activation Functions: relu, logistic, softmax, tanh, or linear";
    }
 ...
 * @return None
 */
void DenseNeuralNetwork::train(vector<vector<double>> data, vector<vector<double>> expected, size_t epochs, double learning_rate, bool (*isAccurate)(vector<double>, vector<double>) = nullptr, bool verbose = true){
    ...
    for (size_t iterations = 0; iterations < epochs; iterations++){
        ...
        for (size_t i = 0; i < data.size(); i++){
            ...
            if (verbose){
                ...
                if (loss == "mean squared error"){
                    //should just be one output node but just in case the values are averaged (not sure if this is good practice)
                    double summing = 0;
                    for (size_t j = 0; j < pred.size(); j++){
                        summing += (expected.at(i).at(j)- pred.at(j))* (expected.at(i).at(j) - pred.at(j));
                    }
                    lossNum += summing/pred.size();
...
vector<double> DenseNeuralNetwork::predict(vector<double> input){ //sofar good
    ...
    for (size_t i = 1; i < layerTotal; i++){
        computeLayer(i, activationPerLayer.at(i-1));   
    }
