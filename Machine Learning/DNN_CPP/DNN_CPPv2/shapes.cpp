...
int main(){
    ...
    vector<size_t> layers4 = {DIM*DIM, DIM*DIM/4, 25, 10, 3};
    vector<string> active = {"logistic", "logistic", "logistic", "softmax"};

    

    try{
        DenseNeuralNetwork dnn("cross entropy", layers4, active);
        
        dnn.train(training_data, training_label, 35 , 0.01, dnn.maxIndx, true); //training data with learning rate of 0.01 on 12 epochs
        cout << endl;
        ...
        dnn.test(testing_data, testing_label, dnn.maxIndx);
        cout << endl;
