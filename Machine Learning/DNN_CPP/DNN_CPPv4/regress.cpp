...
int main(){
    ...
    for(size_t i = 0; i < SIZE; i++){
        ...
        double result = 2*num1 - 3*num2 + noise  + 5;//linear relationship should be close to y = 2a - 3b
    ...
    for(size_t i = 0; i < SIZE/10; i++){
        ...
        double num2 = (double) (rand()%(decimalsplace))/(100000) - 10;
        vector<double> feat = {num1, num2}; 
        double result = 2*num1 - 3*num2 + noise + 5;
        vector<double> label = {result}; 
    ...
    try{
        DenseNeuralNetwork dnn("mean_squared_error", layer, activation);

        dnn.info(); //print weights before training
        dnn.train(train_feature, train_label, 10, 0.01, withinMargin,true,false,1);
        std::string x;
        std::cin >> x;
        dnn.info(); //print weights after training (should be close to 2 and -3)
