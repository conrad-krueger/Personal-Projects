...
Long: I recalled python having a built in sort() algorithm and thinking to myself, why? There has got to be certain scenarios in which 
...
point numbers yet so I didn't bother with Radix. I also didn't bother with merge since while it does have the same complexity, Tim Sort seems to be one of the fastest, if not the fastest, O(n log n) algorithms out there. After running theses tests on different list sizes, Tim Sort always came out ahead. This shocked me
...
Also, its worth noting Quick Sort had awful timing. So I created mevsTim2.py to just focus on Counting Sort.
